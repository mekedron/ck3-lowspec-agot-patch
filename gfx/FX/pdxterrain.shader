...
# gfx/FX/pdxterrain.shader from A Game of Thrones 0.5.2.1 (Workshop id 2962333032),
# itself a modified copy of game/gfx/FX/pdxterrain.shader from CK3 1.19.0.6 (Scribe).
#
...
#       AGOT_ApplyAtmosphericEffects( ... ) in place of ApplyFogOfWar( ... )
...
#   * #define TERRAINOPT_SKIP_HIDDEN_TERRAIN in the PixelShader Code block, and the
#     early out it guards inside PixelShaderLowSpecSharp
# AGOT's own VertexShaderLowSpec / PixelShaderLowSpec blocks are left in place,
# unreferenced, for diffing against future AGOT releases (tools/diff_agot.sh).
#
...
Includes = {
	...
	# MOD(agot)
	#"jomini/jomini_fog_of_war.fxh" #AGOT Disabled
	"agot_atmospheric.fxh" #AGOT Added
	# END MOD
			...
			PDX_MAIN
			{
				...
				float3 Normal = CalculateNormal( Input.WorldSpacePos.xz );
					...
					if( !IsFullyColorOverlay )
					{
						...
						ReorientedNormal = ReorientNormal(
						// END MOD
				...
				if( !IsFullyColorOverlay )
				{
					...
					if( !IsFullyColorOverlay )
					{
						...
						FinalColor = AGOT_ApplyAtmosphericEffects(
							FinalColor,
							Input.WorldSpacePos,
							FogOfWarAlpha,
							// MOD(agot)
							// MOD(agot) dynamic terrain support removed here
							//AGOT_MakeAtmosphericEffectParamsTerrain(TerrainVariantIndex, AdjacentTerrainVariantIndex, AdjacentBlendAmount)
							AGOT_MakeAtmosphericEffectParamsTerrain(0, 0, 0.0f)
							// END MOD
						);
						// END MOD
			...
			PDX_MAIN
			{
				...
				if( !IsFullyColorOverlay )
				{
					...
					if( !IsFullyColorOverlay )
					{
						// MOD(agot)
						//FinalColor = ApplyFogOfWar( FinalColor, Input.WorldSpacePos, FogOfWarAlpha );
						FinalColor = AGOT_ApplyAtmosphericEffects( FinalColor, Input.WorldSpacePos, FogOfWarAlpha, AGOT_MakeAtmosphericEffectParamsGeneric(0) );
						// END MOD
		...
		[[
			PDX_MAIN
			{
				...
				if( !IsFullyColorOverlay )
				{
					...
					if( !IsFullyColorOverlay )
					{
						// AGOT: its atmospheric effects (snowfall + fog of war) in place of ApplyFogOfWar.
						FinalColor = AGOT_ApplyAtmosphericEffects( FinalColor, Input.WorldSpacePos, FogOfWarAlpha, AGOT_MakeAtmosphericEffectParamsGeneric(0) );
						FinalColor = ApplyMapDistanceFog( FinalColor, Input.WorldSpacePos, FogOfWarAlpha );
					}
				#endif
...
Effect PdxTerrain
{
	...
	# MOD(agot)
	#Defines = { "AGOT_ENABLE_DYNAMIC_TERRAIN" }
	# END MOD
}
