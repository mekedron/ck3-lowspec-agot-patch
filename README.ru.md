...
Патч везёт `agot_vic3_fog_of_war.fxh` из AGOT с переключателями в
`gfx/FX/agot_patch_options.fxh`. **По умолчанию слой облаков и тени облаков выключены**,
рисуется только затемнение тумана войны над неизведанной землёй: ноль облачных выборок
на пиксель вместо шести. Переключатели сделаны «удерживающими», так что каждое
умолчание можно отменить вживую из консоли (опция запуска `-debug_mode`), по одному:

| команда консоли | эффект |
| --- | --- |
| `shader_debug AGOTOPT_VANILLA_FOW` | туман войны AGOT ровно как в моде: облака и тени, 6 выборок |
| `shader_debug AGOTOPT_KEEP_CLOUDS` | вернуть слой облаков из одного слоя шума (собственный путь AGOT `LOW_QUALITY_SHADERS`) |
| `shader_debug AGOTOPT_KEEP_CLOUD_SHADOW` | вернуть тени облаков из одного слоя шума |
| `shader_debug AGOTOPT_DIAG_NO_FOW` | диагностика: весь проход выключен, включая затемнение, чтобы замерить его цену |
| `shader_debug` | обратно к умолчаниям |

Чтобы оставить облака или их тени насовсем, раскомментируйте нужный `#define` в файле
опций.
